...
const osThreadAttr_t telem_task_attributes = { .name = "telem_task", .cb_mem =
		&TelemetryControlBlock, .cb_size = sizeof(TelemetryControlBlock),
		.stack_mem = &TelemetryBuffer[0], .stack_size = sizeof(TelemetryBuffer),
		.priority = (osPriority_t) osPriorityNormal, };
/* Definitions for rc_input */
osThreadId_t rc_inputHandle;
const osThreadAttr_t rc_input_attributes = { .name = "rc_input", .stack_size =
		1024 * 4, .priority = (osPriority_t) osPriorityNormal, };
/* Definitions for Buzzer_thread */
osThreadId_t Buzzer_threadHandle;
const osThreadAttr_t Buzzer_thread_attributes = { .name = "Buzzer_thread",
...
int main(void) {
	...
	defaultTaskHandle = osThreadNew(servo_mapping, NULL,
			&defaultTask_attributes);

	/* creation of telem_task */
	telem_taskHandle = osThreadNew(telemetry_task, NULL,
			&telem_task_attributes);

	/* creation of rc_input */
	rc_inputHandle = osThreadNew(arm_disarm, NULL, &rc_input_attributes);

	/* creation of Buzzer_thread */
	Buzzer_threadHandle = osThreadNew(buzzer_task, NULL,
			&Buzzer_thread_attributes);
