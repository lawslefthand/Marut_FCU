 ...
 ******************************************************************************
 * @file           : main.c
 * @brief          : Main program body for MAVLink telemetry with integrated GPS parsing
 ******************************************************************************
 * @attention
 *
...
#include "qmcr5883l.h"
#include <string.h>
#include <math.h>  // For nmeaToDecimal
...
int fix_type = 0;    // MAVLink fix type (computed in gpsParse)
float hdop = 0.0f;   // Horizontal dilution of precision (GPGGA field 8)
uint32_t last_led_toggle = 0;  // For LED throttling based on fix
char unit;  // Altitude unit (M for meters)
uint32_t gps_send_counter = 0; // To throttle GPS MAVLink sends (~1s interval)
...
void send_gps_raw_int(void);  // Added for GPS MAVLink transmission
/* USER CODE END PFP */
...
void bmp_hal_i2c_write(uint8_t reg_addr, uint8_t value) {
	...
	HAL_I2C_Mem_Write(&hi2c3, BMP280_I2C_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT,
			&data_to_write, 1, HAL_MAX_DELAY);
}

uint8_t bmp_hal_i2c_read(uint8_t reg_addr) {
	uint8_t read_value = 0;
	HAL_I2C_Mem_Read(&hi2c3, BMP280_I2C_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT,
...
int main(void) {
	...
	while (1) {
		...
		/*
		 ...
		 // Throttle GPS send (~ every 20 loops at 50ms = 1s)
		 gps_send_counter++;
		 if (gps_send_counter >= 20) {
		 send_gps_raw_int();  // Send GPS even if no fix (fix_type=0)
		 gps_send_counter = 0;
		 }

		 // LED control based on fix status (imported from code 1)
		...
		HAL_Delay(50);  // Unchanged loop delay
		/* USER CODE END WHILE */
