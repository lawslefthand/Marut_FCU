...
# define TS 0.004f
# define sd_baro 30.0f
# define sd_accZ 10.0f
/* USER CODE END PD */
...
/* USER CODE BEGIN PV */
float AccX;
float AccY;				// from accelerometer in g
float AccZ;
float AngleRoll;
float AnglePitch;
float Altitude_baro;  // from barometer in cm

// below are the varaibles for kalman filter
...
float getAccZInertial(float AccX, float AccY, float AccZ, float AngleRoll,
		float AnglePitch);
void kalman_2d_step(float acc, float meas); // acc is output of function getAccZInertial, meas is Altitude_baro
...
float getAccZInertial(float AccX, float AccY, float AccZ, float AngleRoll,
		...
		float AccZ_world = -sinf(pitch) * AccX + cosf(pitch) * sinf(roll) * AccY
				+ cosf(pitch) * cosf(roll) * AccZ;
		float AccZInertial = (AccZ_world - 1.0f) * 9.81f * 100.0f;
		return AccZInertial;
	}
void kalman_2d_step( float acc, float meas) {
		...
		float L = P11p + R;
		if (L <= 1e-12f)
			L = 1e-12f; // guard against div by zero
		...
		float newP21 = P21p - K1 * P11p;
		float newP22 = P22p - K1 * P12p;
...
int main(void) {
	...
	while (1) {
		/* USER CODE END WHILE */
		float Accel_mpu = getAccZInertial(AccX, AccY, AccZ,
		                                          AngleRoll, AnglePitch);

		        kalman_2d_step(Accel_mpu, Altitude_baro);
